...
st.set_page_config(page_title="Installment Projections", layout="centered")

st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap');
*{box-sizing:border-box;}
html,body,[class*="css"]{font-family:'Inter',sans-serif;color:#111;}
.stApp{background:#F0F2F5;}
#MainMenu,footer,header{visibility:hidden;}
.block-container{max-width:560px!important;padding:4rem 1rem 2rem!important;}

/* Card */
.card{background:white;border-radius:16px;padding:2rem 2rem 2rem;box-shadow:0 1px 3px rgba(0,0,0,.07),0 6px 24px rgba(0,0,0,.07);}
.card-title{font-size:1.3rem;font-weight:700;color:#111;margin:0 0 .35rem;}
.card-sub{font-size:.875rem;color:#6B7280;line-height:1.55;margin:0 0 1.5rem;}
.section-label{font-size:.68rem;font-weight:600;letter-spacing:1.8px;text-transform:uppercase;color:#9CA3AF;margin-bottom:.45rem;}

/* File uploader */
[data-testid="stFileUploader"]{background:#F9FAFB;border:1.5px dashed #CBD5E1;border-radius:10px;padding:.25rem .75rem .75rem;transition:border-color .2s;}
[data-testid="stFileUploader"]:hover{border-color:#94A3B8;}
[data-testid="stFileUploaderDropzone"]{padding:.5rem 0;}

/* Results card */
.result-card{background:white;border-radius:14px;padding:1.4rem 1.6rem 1.2rem;box-shadow:0 1px 3px rgba(0,0,0,.07),0 4px 16px rgba(0,0,0,.06);margin:1.25rem 0;}
.result-name{font-size:.95rem;font-weight:600;color:#111;margin:0 0 .15rem;}
.result-meta{font-size:.78rem;color:#9CA3AF;margin:0 0 1rem;}

/* Stats */
.stats-row{display:flex;gap:.6rem;margin:0 0 1.1rem;}
.stat-card{flex:1;background:#F9FAFB;border-radius:8px;padding:.7rem .4rem;text-align:center;border:1px solid #E5E7EB;}
.stat-card .stat-num{font-size:1.25rem;font-weight:700;color:#111;line-height:1;}
.stat-card .stat-lbl{font-size:.6rem;color:#9CA3AF;text-transform:uppercase;letter-spacing:1px;margin-top:.2rem;}

/* Download button */
[data-testid="stDownloadButton"]>button{background:#374151!important;color:white!important;border:none!important;border-radius:8px!important;padding:.65rem 1.25rem!important;font-family:'Inter',sans-serif!important;font-weight:600!important;font-size:.875rem!important;width:100%!important;transition:background .15s!important;}
[data-testid="stDownloadButton"]>button:hover{background:#1F2937!important;}

/* Error */
.err-box{background:#FEF2F2;border:1px solid #FECACA;border-radius:8px;padding:.85rem 1rem;color:#B91C1C;font-size:.85rem;margin:.75rem 0;}

/* Divider between results */
.result-divider{border:none;border-top:1px solid #F3F4F6;margin:1rem 0;}
</style>
""", unsafe_allow_html=True)

# ── CONSTANTS ─────────────────────────────────────────────────────────────────

PRIMARY_ACCOUNTS = {'Slash','Divvy Credit','Divvy Prefund','Wex Credit','Wex Prefund','Global Rewards','Taekus'}
...
st.markdown("""
<div class="card">
    <div class="card-title">Installment Projections</div>
    <div class="card-sub">Upload the QBO ticket credit general ledger (one company or consolidated) for any time period in the prior month to project the same period in the coming month.</div>
    <div class="section-label">GL Files</div>
""", unsafe_allow_html=True)
...
if uploaded_files:
    for uploaded in uploaded_files:
        with st.spinner(f"Building {uploaded.name}…"):
            try:
                ...
                <div class="result-card">
                    ...
                    <div class="result-meta">{stats['date_range']} · {co_str}{stats['transactions']:,} transactions</div>
                    <div class="stats-row">
                ...
                st.download_button(label=f"Download Report",data=xlsx_bytes,file_name=filename,
